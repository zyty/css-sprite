...
var url = require('url');
var cdn = require('qcdn');

// json2css template
json2css.addTemplate('sprite', require(path.join(__dirname, 'templates/sprite.js')));

module.exports = function (opt) {
  opt = lodash.extend({}, {name: 'sprite', format: 'png', margin: 4, processor: 'css', cssPath: '../images', orientation: 'vertical', sort: true, interpolation: 'grid', cdn: false}, opt);
    ...
    async.waterfall([
      ...
      function (sprite, nonRetinaSprite, callback) {
        sprite.canvas.toBuffer(opt.format, {}, function (err, buf) {
          if(opt.cdn){
            // console.log(buf);
            cdn(buf.toString('binary'), 'media').then(function (url) {
              sprite.relative = url;
              callback(null, sprite, nonRetinaSprite);
              // body...
            }).catch(function (err) {
